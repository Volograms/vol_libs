...
bool create_file_info(const char *hdr_filename, const char *seq_filename)
{
	...
	return vol_geom_create_file_info(
			hdr_filename,
			seq_filename, &_info, true);
}
...
{
	return vol_geom_find_previous_keyframe(&_info, frame_idx);
}

EMSCRIPTEN_KEEPALIVE
uint8_t* frame_vertices() {
	return &_frame_data.block_data_ptr[_frame_data.vertices_offset];
}

EMSCRIPTEN_KEEPALIVE
int32_t frame_vertices_sz() {
	return _frame_data.vertices_sz;
}

EMSCRIPTEN_KEEPALIVE
int32_t frame_uvs_sz() {
	return _frame_data.uvs_sz;
}
...
EMSCRIPTEN_KEEPALIVE
int32_t frame_i_sz() {
	return _frame_data.indices_sz;
}

EMSCRIPTEN_KEEPALIVE
uint8_t* frame_data_ptr() {
	return _frame_data.block_data_ptr;
}

EMSCRIPTEN_KEEPALIVE
uint32_t frame_vp_offset() {
	return _frame_data.vertices_offset;
}
